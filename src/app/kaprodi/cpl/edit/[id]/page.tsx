...
export default function EditCPLPage() {
  ...
  const validateDataIntegrity = (data: UpdateCPLRequest): { isValid: boolean; message?: string } => {
    ...
    if ((data.kode?.length || 0) > 50 || (data.nama?.length || 0) > 500 || (data.deskripsi?.length || 0) > 2000) {
      return { isValid: false, message: 'Data terlalu panjang' }
    }
    
    // Check for empty strings after trimming
    if ((data.kode && !data.kode.trim()) || (data.nama && !data.nama.trim()) || (data.deskripsi && !data.deskripsi.trim())) {
  ...
  const handleSave = async (isDraft = false) => {
    ...
    if (authUser && authUser.role !== 'kaprodi') {
      setSubmitError('Anda tidak memiliki izin untuk mengedit CPL')
      return
    }
    ...
    try {
      ...
      const updateData: UpdateCPLRequest = trimmedData

      // Validate data integrity
      const integrityCheck = validateDataIntegrity(updateData)
      if (!integrityCheck.isValid) {
        setSubmitError(integrityCheck.message || 'Data tidak valid')
        return
      }

      console.log('Updating CPL data:', updateData) // For debugging
